...
export function getBashPathForPlatform(params: GetBashPathForPlatformParams): string {
  ...
  if (!bashPath) {
    throw new Error(
      "Git Bash not found. On Windows, Steward requires Git for Windows (Git Bash). WSL is not supported. Install Git for Windows from https://git-scm.com/download/win"
    );
  }
