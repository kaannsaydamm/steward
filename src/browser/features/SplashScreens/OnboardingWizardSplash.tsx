...
export function OnboardingWizardSplash(props: { onDismiss: () => void }) {
  ...
  const steps = useMemo((): WizardStep[] => {
    ...
    if (hasConfiguredProvidersAtStart === false) {
      nextSteps.push({
        key: "mux-gateway",
        title: "Hosted gateway (evaluation credits)",
        icon: <Sparkles className="h-4 w-4" />,
        body: (
          <>
            <p>
              The hosted gateway enables you to use evaluation AI credits from{" "}
              <a
              ...
              <div className="mt-3 space-y-2">
                <div className="border-border-medium bg-background-secondary rounded-md border p-2 text-xs">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="text-foreground font-medium">Hosted gateway account</div>
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => {
                ...
                {muxGatewayAccountStatus?.remaining_microdollars === 0 && (
                  <div className="border-destructive/20 bg-destructive/5 mt-2 rounded-md border p-2 text-xs">
                    <p className="text-destructive font-medium">
                      Your hosted gateway credits are depleted.
                    </p>
    ...
    nextSteps.push({
      ...
      body: (
        <>
          <p>
            Steward is provider-agnostic: bring your own keys, mix and match models, or run locally.
          </p>
    ...
    nextSteps.push({
      ...
      body: (
        <>
          <p>
            Projects are the folders or repos you want Steward to work in. Add a local folder or
            clone from GitHub, then click Next.
          </p>
      ...
      body: (
        <>
          <p>
            ...
            agents in <code className="text-accent">~/.steward/app/agents/*.md</code>.
          </p>
        ...
        <>
          ...
          <div className="mt-3 grid gap-2">
            ...
            <Card icon={<WorktreeIcon size={14} />} title="Worktree">
              Isolated git worktree under <code className="text-accent">~/.steward/app/src</code>.
            </Card>
    ...
    nextSteps.push({
      ...
      body: (
        <>
          <p>
            MCP servers extend Steward with tools (memory, ticketing, databases, internal APIs).
          ...
          <div className="mt-3 grid gap-2">
            <Card icon={<Server className="h-4 w-4" />} title="Global config">
              <code className="text-accent">~/.steward/app/mcp.jsonc</code>
            </Card>
            <Card icon={<Server className="h-4 w-4" />} title="Repo overrides">
              <code className="text-accent">./.steward/mcp.jsonc</code>
            </Card>
            <Card icon={<Server className="h-4 w-4" />} title="Workspace overrides">
              <code className="text-accent">.steward/mcp.local.jsonc</code>
            </Card>
          </div>
